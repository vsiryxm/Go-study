...
func main() {
    //第一种定义方法：
    s := []int {0, 1, 2, 3, 4, 5, 6, 7, 8} //完整的写法：var s []int = []int {0, 1, 2, 3, 4, 5, 6, 7, 8}，但go讲究简洁
    printSlice("s", s)

    //第二种定义方法：
    var s2 = make([]int, 3, 5) //make(类型，长度，容量)
    printSlice("s2", s2)
    ...
    fmt.Println("s[1:4] ==", s[1:4]) //从下标（或称索引）为1的元素开始截取，截取到第4个元素（不包含），即截取4-1个
    ...
    fmt.Println("\n---切片扩容操作---")
    s = append(s, 9) //上一次s的长度为9，容量为9，现在加入1个元素后，容量不够用了，扩展到len*2，长度+1
    printSlice("s", s)
    s = append(s, 10,11,12,13,14,15,16,17,18) //上一次s的长度为10，容量为18，现在加入9个元素后，容量不够用了，扩展到上一次容量*2，长度+1
    printSlice("s", s)
