...
//参考理解：https://www.cnblogs.com/bonelee/p/6862377.html
...
func main() {
	...
	fmt.Println(&s);
	//printSlice("s", s)
	//slice := s[:] //[low:high:max] len=high-low cap=max-low
	//fmt.Printf("slice的长度len=%d，容量cap=%d\n", len(slice), cap(slice))
	//slice2 := slice[9:9]
	//fmt.Println(slice2)
	...
	//fmt.Println("s[:3] ==", s[:3])   //省略上限，则从0开始，容量索引号即为s的长度
