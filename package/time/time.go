...
func Time() {
	...
	ss := t.Unix()      //单位：秒s
	ms := t.UnixMilli() //单位：毫秒ms
	us := t.UnixMicro() //单位：微秒μs
	ns := t.UnixNano()  //单位：纳秒ns

	fmt.Printf("当前时间戳（10位，即秒s）为：%d \n", ss)
	//结果：当前时间戳（10位，即秒s）为：1572077874
	fmt.Printf("当前时间戳（13位，即毫秒ms）为：%d \n", ms)
	//结果：当前时间戳（13位，即毫秒ms）为：1572077874913
	fmt.Printf("当前时间戳（16位，即微秒μs）为：%d \n", us)
	//结果：当前时间戳（16位，即微秒μs）为：1572077874913328
	fmt.Printf("当前时间戳（19位，即纳秒ns）为：%d \n", ns)
	//结果：当前时间戳（19位，即纳秒ns）为：1572077874913328300
	...
	// 将带有时区格式的日期字符串转换成time.Time格式
	timeStr := "2023-08-08T18:16:33+08:00"
	t2, _ := time.Parse("2006-01-02T15:04:05Z", timeStr)
	localTimeZone, _ := time.LoadLocation("Local")
	expiration := t2.In(localTimeZone)
	fmt.Printf("将带有时区格式的日期字符串转换成time.Time格式：%v\n", expiration)
